...
public class TextLayout {
    ...
    public TextLayout(CharSequence text, Font font, Alignment alignment, float lineLength, int colour) {
        ...
        while (true) {
            if (end < text.length() && text.charAt(end) != '\n') {
                ...
                start = end;
                if (end >= text.length()) {
                    this.textShape = tsb.mo82fuse();
                    this.lines = (CharSequence[]) linesList.toArray(new CharSequence[linesList.size()]);
                    return;
                }
            }
        }
    ...
    private static CharSequence[] splitParagraph(CharSequence text, Font f, float lineLength) {
        int increment;
        ...
        return (CharSequence[]) lines.toArray(new CharSequence[lines.size()]);
    }

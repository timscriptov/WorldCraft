...
public class BedBlock {
    ...
    public static boolean isBed(BlockFactory.Block b) {
        return b != null && isBed(Byte.valueOf(b.id));
    }

    public static boolean isBed(Byte blockType) {
        return blockType != null && blockType.byteValue() == 75;
    }
    ...
    public static void renderPreview(World world, StackedRenderer renderer) {
        ...
        boolean isPillow = (previewBlockData.byteValue() & 8) == 8;
        byte direction = (byte) (previewBlockData.byteValue() & 3);
        switch (direction) {
            case 0:
    ...
    public static void generateGeometry(Chunklet c, ShapeBuilder opaqueShapeBuilder, int x, int y, int z, byte data, int colour) {
        ...
        Parallelepiped p = isPillow ? bedWithPillow.m84clone() : bedWithOutPillow.m84clone();
        if (direction == 0 && !isPillow && !isBed(Byte.valueOf(c.blockType(x, y, z - 1))) && !isBed(Byte.valueOf(c.blockType(x, y, z + 1)))) {
    ...
    public static void breakBed(Chunk chunk, Vector3i blockLocation, Player player) {
    ...
    public static Vector3i getBedLeftBlockLocation(Chunk chunk, Vector3i blockLocation) {
        int x = blockLocation.x;
        int y = blockLocation.y;
        int z = blockLocation.z;
        byte blockType = chunk.blockTypeForPosition(x, y, z);
        if (isBed(Byte.valueOf(blockType))) {
            byte leftBlockType = chunk.blockTypeForPosition(x - 1, y, z);
            if (isBed(Byte.valueOf(leftBlockType))) {
    ...
    public static boolean placeBed(Chunk chunk, Vector3i placementTargetBlock, InventoryItem invItem, Player player) {
    ...
    private static void addFace(Parallelepiped facing, float x, float y, float z, Parallelepiped.Face f, float width, float height, int colour, ShapeBuilder shapeBuilder, boolean invertCoords) {

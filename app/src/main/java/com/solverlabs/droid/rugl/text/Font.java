...
public final class Font {
    ...
    public Font(ByteBuffer data) {
        ...
        this.distanceField = data.get() == 0 ? false : z;
        int gi = data.getInt();
        for (int i = 0; i < gi; i++) {
            this.glyphImages.add(new GlyphImage(data));
        }
        GlyphImage[] imageArray = (GlyphImage[]) this.glyphImages.toArray(new GlyphImage[this.glyphImages.size()]);
        int g = data.getInt();
    ...
    public Font(InputStream is) throws IOException {
        ...
        this.distanceField = dis.readByte() == 0 ? false : z;
        int gi = dis.readInt();
        for (int i = 0; i < gi; i++) {
    ...
    public boolean init(boolean mipmap) {
        ...
        if (this.texture == null) {
            ...
            gImages.addAll(this.glyphImages);
    ...
    public TextShape buildTextShape(CharSequence text, int colour) {
        if ($assertionsDisabled || this.texture != null) {
            ...
            TexturedShape ts = new TexturedShape(new ColouredShape(new Shape(verts, indices), colour, (State) null), texcoords, this.texture.getTexture());

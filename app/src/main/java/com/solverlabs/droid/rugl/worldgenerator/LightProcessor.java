...
public class LightProcessor {
    ...
    public void recalculateSkyLightingAround(Chunk chunk, int x, int y, int z) {
        int light = 0;
        try {
    ...
    private void recalculateSkyLightRecursive(Chunk chunk, int x, int y, int z) {
    ...
    private boolean isSunlitBlock(Chunk chunk, int x, int y, int z) {
        return chunk.skyLight(x, y + 1, z) == 15 && chunk.blockType(x, y + 1, z) == 0;
    }

    public void recalculateBlockLightAround(Chunk chunk, int x, int y, int z) {
        try {
            int wx = (chunk.chunkX * 16) + x;
            int wz = (chunk.chunkZ * 16) + z;
    ...
    private void setBlockLightRecursive(Chunk chunk, int x, int y, int z) {
        int wx = (chunk.chunkX * 16) + x;
        int wz = (chunk.chunkZ * 16) + z;
        Chunklet dirty = this.world.getChunklet(wx, y, wz);
        if (dirty != null) {

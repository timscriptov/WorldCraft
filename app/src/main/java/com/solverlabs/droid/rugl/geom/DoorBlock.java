...
public class DoorBlock {
    ...
    public static void renderPreview(World world, StackedRenderer renderer) {
        byte previewBlockData = world.getPreviewBlockData().byteValue();
    ...
    public static void generateGeometry(Chunklet c, ShapeBuilder opaqueShapeBuilder, int x, int y, int z, byte blockId, int colour, byte data) {
        ...
        if (!isUpperBlock && isDoor(Byte.valueOf(c.blockType(x, y + 1, z)))) {
            addFaces(c, x, y, z, colour, opaqueShapeBuilder, blockId, data);
        }
    }

    public static void actionDoor(byte blockType, World world, Vector3i blockLocation) {
        ...
        byte data = world.getBlockDataAbsolute(blockLocation.x, blockLocation.y, blockLocation.z).byteValue();
        SoundManager.playDoorChangeType(isDoorOpen(data), 0.0f);
    }

    private static void rotateDoorBlock(Chunk chunk, Vector3i blockLocation, byte blockType) {
        ...
        if (isDoor(Byte.valueOf(bottomBlockType)) && !isUpperDoorBlock(bottomBlockData)) {
            data = (byte) (data | 8);
    ...
    public static void breakDoor(Chunk chunk, Vector3i blockLocation) {
    ...
    public static Vector3i getDoorDownBlockLocation(Chunk chunk, Vector3i blockLocation) {
        int x = blockLocation.x;
        int y = blockLocation.y;
        int z = blockLocation.z;
        byte blockType = chunk.blockTypeForPosition(x, y, z);
        if (isDoor(Byte.valueOf(blockType))) {
            byte downBlockType = chunk.blockTypeForPosition(x, y - 1, z);
            if (isDoor(Byte.valueOf(downBlockType))) {
    ...
    public static boolean placeDoor(Chunk chunk, Vector3i placementTargetBlock, InventoryItem invItem, Player player) {
    ...
    public static void updateBlockBounds(BoundingCuboid blockBounds, float x, float y, float z, World world) {
        Byte data = world.getBlockDataAbsolute((int) x, (int) y, (int) z);
        if (data != null) {
            boolean isOpen = isDoorOpen(data.byteValue());
            byte direction = (byte) (data.byteValue() & 3);
            switch (direction) {
                case 0:
                    if (isOpen) {
    ...
    private static void addFaces(Chunklet c, int x, int y, int z, int colour, ShapeBuilder opaque, byte id, byte data) {
        Parallelepiped p = getParallelepiped(id).m84clone();
    ...
    public static boolean isDoor(BlockFactory.Block b) {
        return b != null && isDoor(Byte.valueOf(b.id));
    }

    public static boolean isDoor(Byte blockType) {
        return blockType != null && (blockType.byteValue() == 62 || blockType.byteValue() == 64 || blockType.byteValue() == 65 || blockType.byteValue() == 66);
    }

...
public class Furnace extends TileEntity {
    ...
    public void initFurnace() {
        ...
        float usedMaterial = (((float) (this.cookTime + timeDelta)) * 1.0f) / 10000.0f;
        if (usedMaterial > materialCount) {
            float usedMaterial2 = materialCount;
        }
        float usedFuel = (((float) (timeDelta - this.burnTime)) * 1.0f) / this.maxBurnTime;
        if (usedFuel > fuelCount) {
            float usedFuel2 = fuelCount;
        }

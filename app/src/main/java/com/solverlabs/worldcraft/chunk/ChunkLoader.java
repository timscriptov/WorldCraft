...
public abstract class ChunkLoader extends ResourceLoader.Loader<Chunk> {
    ...
    public void load() {
        try {
            DataInputStream is = RegionFileCache.getChunkDataInputStream(this.world.mDir, this.x, this.z);
            if (is != null) {
                this.resource = new Chunk(this.world, is);
                if (this.resource != null && (this.resource.chunkX != this.x || ((Chunk) this.resource).chunkZ != this.z)) {
            ...
            if (this.resource == null && !GameMode.isMultiplayerMode() && this.world.getMapType() != -1 && GameMode.isSurvivalMode()) {
                ...
                if (this.world.getMapType() == 0) {
                    this.world.generateTerrain((Chunk) this.resource, false);
                }
                if (this.world.getMapType() == 1) {
                    this.world.generateTerrain((Chunk) this.resource, true);
                }
            }
        } catch (Exception e) {
